...
public class ServiceResource(ICoreService<Resource> coreService, IValidator<Resource> resourceValidator) : IServiceResource
{
    ...
    /// <inheritdoc />
    public async Task<ICollection<ResponseResourceDto>> GetAllByResourceTypeIdAsync(long resourceTypeId)
    {
        var spec = new BaseSpecification<Resource>(x => x.ResourceTypeId == resourceTypeId);
